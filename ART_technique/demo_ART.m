...
%% DATA GENERATION
load('XCAT512.mat');
x       = imresize(double(XCAT512), [N, N]); % ground truth 
p       = A(x); % projection of the intensity with radon transform 
x_full  = AINV(p); % same as ground truth, just passed by radon and invradon 

%% LOW-DOSE SINOGRAM GENERATION

% Simulates a low dose acquisition 
i0     	= 5e4;
pn     	= exp(-p);
pn     	= i0.*pn;
pn     	= poissrnd(pn);
...
x_low   = AINV(y); % low dose image reconstructed  
x0      = zeros(size(x));
lambda  = 1e0;
niter   = 2e2;
...
x_art   = ART(A, AT, y, x0, lambda, niter, bfig); % reconstructed 
