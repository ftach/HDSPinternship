...
def UNetL(pretrained_weights = None,input_size = (256,256,1), L=10, H=0, fact = 0.5,F=0,shots_hs=1,shots_ms=1,shots_per_eval=1,y=None):
    ...
    for i in eval_idx:
        # print(y[i].shape,yp[i].shape)
        loss += tf.reduce_mean(tf.square(y[i] - F[i](conv8)[0])) # E(y - H*N(Z)) with N(Z) = conv8
        print(loss)
    loss = loss / shots_per_eval
    model.add_loss(loss)
    # self.add_metric(norm, name='loss',aggregation='mean')
...
def Auto_encoder(pretrained_weights = None,input_size = (256,256,1), L=10, H=0, fact = 0.5,F=0,shots_hs=1,shots_ms=1,shots_per_eval=1,y=None):
    ...
    inputs = Input(input_size)    
    inicial = XoLayer(largo=input_size[0], ancho=input_size[1], profun=L, fact=fact)(inputs) # Tucker input layer
    ...
    conv8=Conv2DTranspose(filters=L, kernel_size=(3, 3), strides=2, activation='relu', padding='same')(convup3) # Output layer 

    model = Model(inputs, conv8)

    # mse = tf.keras.losses.MeanSquaredError()
    # Getting the random subsets of measurements to compute the loss
    eval_idx = np.random.permutation(shots_hs + shots_ms)[:shots_per_eval]
    loss = 0
    for i in eval_idx:
        # print(y[i].shape,yp[i].shape)
        loss += tf.reduce_mean(tf.square(y[i] - F[i](conv8)[0])) # loss isn't computed with a ground truth
    loss = loss / shots_per_eval
    model.add_loss(loss)
    # self.add_metric(norm, name='loss',aggregation='mean')
